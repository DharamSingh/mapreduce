...
private static final String[] bucket = {
 ...
 private static void doInserts() throws Exception {
   ...
   // XXX lotta for loops below, this is just hack demo code
   // create some date for each year, so regions for that year aren't
   // just blank
 ...
 private static Put createNewEvent(String date) {
   ...
   Integer bucket = (Math.abs(uuid.hashCode()) % 10);
   Put row = new Put(Bytes.toBytes(String.format("%s:%s:%s",bucket,
date, uuid)));
 ...
 private static void createTable() throws Exception {
  ...
  byte[][] splitKeys = new byte[bucket.length][];
   for (int i = 0; i < bucket.length; i++) {
     splitKeys[i] = bucket[i].getBytes();
   }
